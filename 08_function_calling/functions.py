...
def df_as_text(df):
    ...
    # Normalize input to a DataFrame so callers can pass lists/dicts/scalars.
    ...
    # Convert DataFrame to markdown table when possible.
    # Fallback to plain text when optional dependency `tabulate` is unavailable.
    try:
        tab = table_df.to_markdown(index=False)
    except ImportError:
        tab = table_df.to_string(index=False)
    return tab
